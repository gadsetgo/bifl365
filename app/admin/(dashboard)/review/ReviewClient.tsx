...
export function ReviewClient({ initialProducts }: { initialProducts: Product[] }) {
  ...
  // Local drafted state for the current active card
  ...
  // AI Tone Matrix & Source Links state
  const [activeTones, setActiveTones] = useState<string[]>(['reddit']);
  const [sourceLinks, setSourceLinks] = useState(''); 

  // Reset drafted state whenever the active product changes
  useEffect(() => {
    if (activeProduct) {
  ...
  // Aggregate current draft for save
  function draftedUpdates() {
    return {
      name: draftName,
      description_draft: draftDesc,
      summary: draftDesc, // Copying the approved draft to frontend summary
      award_type: draftAward === 'none' ? null : draftAward,
      amazon_tag: 'bifl365-21' // The user's active affiliate tag
    };
  }
  ...
  async function handleAI() {
    ...
    try {
      ...
      if (data?.newDescription) {
        setDraftDesc(data.newDescription);
      }
    } catch(err) {
      console.error(err);
  ...
  if (!activeProduct) {
    return (
      <div className="py-20 text-center border border-charcoal bg-white shadow-card">
        ...
        <p className="text-sm font-sans text-charcoal-400">All caught up. Trigger a new manual pipeline to find products.</p>
      </div>
    );
  }
  ...
  return (
    <div className="relative">
      <div className="mb-4 text-xs font-sans font-bold text-charcoal-400 uppercase tracking-widest flex items-center justify-between">
        <span>Reviewing {currentIndex + 1} of {queue.length}</span>
        <button onClick={() => setCurrentIndex(p => p + 1)} className="hover:text-orange transition-colors">Skip (Tab) →</button>
      </div>
      ...
      <div className="bg-white border border-charcoal p-6 lg:p-8 shadow-[8px_8px_0px_0px_#121212] flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-paper-dark p-4 border border-ghost flex flex-col gap-2">
             <span className="text-2xs font-sans uppercase tracking-widest text-charcoal-400">Scores (Read-only for now)</span>
             <ul className="text-xs font-sans text-ink space-y-1">
               <li className="flex justify-between"><span>Build Quality</span> <strong>{activeProduct.scores?.build_quality}/20</strong></li>
               <li className="flex justify-between"><span>Longevity</span> <strong>{activeProduct.scores?.longevity}/20</strong></li>
               <li className="flex justify-between"><span>Value</span> <strong>{activeProduct.scores?.value}/20</strong></li>
               <li className="flex justify-between"><span>Repairability</span> <strong>{activeProduct.scores?.repairability}/20</strong></li>
             </ul>
          </div>
        </div>
        ...
        <div className="space-y-4">
           <div className="flex flex-wrap lg:flex-nowrap items-start gap-4">
              ...
              <div className="flex-1 space-y-2">
                 <label className="block text-2xs font-sans uppercase tracking-widest text-charcoal-400 mb-1">Source Quote & Link Integrator</label>
                 <textarea 
        ...
        {/* Action Bar */}
        <div className="flex items-center gap-4 mt-4 pt-4 border-t border-ghost">
           <button 
